...
const METHODS: { method: string; description: string; safe: string; idempotent: string }[] = [
  { method: "GET", description: "Retrieve a resource. Should not modify server state.", safe: "Yes", idempotent: "Yes" },
  ...
  { method: "PATCH", description: "Partial update to a resource. Not idempotent (usually).", safe: "No", idempotent: "No" },
...
export default function HttpMethodsRef() {
  ...
  const filtered = useMemo(
    () => search.trim() ? METHODS.filter((m) => m.method.includes(search.toUpperCase()) || m.description.toLowerCase().includes(search.toLowerCase())) : METHODS,
    [search]
  );
  return (
    <Form>
      ...
      {filtered.map((m) => (
        <Form.Description key={m.method} title={m.method} text={`${m.description} | Safe: ${m.safe} | Idempotent: ${m.idempotent}`} />
      ))}
    </Form>
  );
}
