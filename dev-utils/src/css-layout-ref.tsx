...
const ITEMS = [
  ...
  { prop: "justify-content", values: "flex-start | center | flex-end | space-between | space-around | space-evenly", desc: "Alignment along main axis" },
  { prop: "align-items", values: "stretch | flex-start | center | flex-end | baseline", desc: "Alignment along cross axis" },
  { prop: "align-self", values: "auto | stretch | flex-start | center | flex-end | baseline", desc: "Override align-items for one item" },
  ...
  { prop: "place-content", values: "<align-content> / <justify-content>", desc: "Shorthand for align and justify content" },
];

export default function CssLayoutRef() {
  const [search, setSearch] = useState("");
  const filtered = useMemo(
    () => search.trim() ? ITEMS.filter((i) => i.prop.includes(search) || i.desc.toLowerCase().includes(search.toLowerCase())) : ITEMS,
    [search]
  );
