...
function generateTable(): string {
  ...
  for (let row = 0; row < 32; row++) {
    ...
    for (let block = 0; block < 4; block++) {
      ...
      const char = code <= 0x7f ? (code >= 0x20 && code !== 0x7f ? String.fromCodePoint(code) : code === 0x20 ? "␣" : "␀") : "-";
      cols.push(
        `${String(code).padStart(3)}  ${code.toString(16).toUpperCase().padStart(2)}  ${char.padEnd(3)}`
      );
    }
