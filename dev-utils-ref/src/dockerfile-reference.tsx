...
const INSTRUCTIONS: { instr: string; description: string; example: string }[] = [
  ...
  {
    instr: "ENTRYPOINT",
    description: "Main command (harder to override)",
    example: 'ENTRYPOINT ["docker-entrypoint.sh"]',
  },
  {
    instr: "LABEL",
    description: "Add metadata (key=value)",
  ...
  {
    instr: "ADD",
    description: "Copy files (with URL/tar support)",
  ...
  { instr: "SHELL", description: "Override default shell", example: 'SHELL ["/bin/bash", "-c"]' },
  {
    ...
    example: "HEALTHCHECK --interval=30s CMD curl -f http://localhost/",
  },
...
export default function DockerfileReference() {
  ...
  return (
    <Form>
      <Form.TextField id="search" title="Search" placeholder="FROM, RUN, CMD…" value={search} onChange={setSearch} />
      {filtered.map((i) => (
