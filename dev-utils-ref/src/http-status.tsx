...
const STATUS_CODES: Record<number, { title: string; description: string }> = {
  ...
  101: {
    ...
    description: "The requester has asked the server to switch protocols and the server has agreed.",
  },
  ...
  304: {
    title: "Not Modified",
    description: "Indicates the resource has not been modified since the version specified by If-Modified-Since.",
  },
...
export default function HttpStatus() {
  ...
  const filtered = useMemo(() => {
    return Object.entries(STATUS_CODES)
      .filter(([code, info]) => {
        ...
        return (
          code.includes(search) || info.title.toLowerCase().includes(q) || info.description.toLowerCase().includes(q)
        );
      })
      .map(([code, info]) => ({
        code: Number(code),
        ...info,
        category: code.startsWith("1")
          ? "1xx Informational"
          : code.startsWith("2")
            ? "2xx Success"
            : code.startsWith("3")
  ...
  return (
    ...
    >
      <Form.TextField id="search" title="Search" placeholder="Code or name…" value={search} onChange={setSearch} />
      {filtered.map((s) => (
        <Form.Description key={s.code} title={`${s.code} ${s.title}`} text={`${s.category} — ${s.description}`} />
      ))}
    </Form>
  );
}
